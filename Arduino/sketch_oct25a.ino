...
TrellisCallback keyPressCallback(keyEvent evt) {
  ...
  if (evt.bit.EDGE == SEESAW_KEYPAD_EDGE_RISING) {
    if (currentTime - lastPressTime[evt.bit.NUM] > debounceDelay) {
      ...
      trellis.pixels.setPixelColor(evt.bit.NUM, trellis.pixels.Color(255, 255, 255)); // 흰색
      trellis.pixels.show();
    }
  } 
...
void loop() {
  if (client.available()) {
    ...
    if (received == "activate_keypad") {
      ...
      for (uint8_t i = 0; i < 16; i++) {
        trellis.pixels.setPixelColor(i, trellis.pixels.Color(255, 255, 255)); // 흰색
      }
      trellis.pixels.show(); 
    } else if (received == "failure") { //얼굴인식 실패 신호를 전달받으면 1분동안 부저음만 울리게 하기
        isDeviceEnabled = false;
        Serial.println("장치 비활성화됨");
        playTone('5');
        for (uint8_t i = 0; i < 16; i++) {
          trellis.pixels.setPixelColor(i, 0); // LED를 모두 끔
        }
        trellis.pixels.show();
    }
...
void handleKeyPress(char key) {
  if (key == '*') {
    ...
    if (inputPassword == correctPassword) {
      ...
      fail++;
      for (uint8_t i = 0; i < 16; i++) {
        trellis.pixels.setPixelColor(i, 255, 255, 255); // 모든 LED를 흰색으로 설정 (RGB: 255, 255, 255)
      }
      trellis.pixels.show(); // 변경 사항 적용
      ...
      for (uint8_t i = 0; i < 16; i++) {
        trellis.pixels.setPixelColor(i, 255, 255, 255); // 모든 LED를 흰색으로 설정
      }
      trellis.pixels.show(); // 변경 사항 적용
    }
    inputPassword = "";
...
void playTone(char result){
  ...
  //이부분 신규추가 -> 얼굴인식이나 키패드가 5번 이상 틀렸을땐 1분간 홀드랑 부저음이 필요함
  else if(result == '5'){
    for(int i = 0; i < 10 ; i++){ //우선 10번만 실행하게 하기
      tone(BUZZER, 294, 200);
      delay(100);
      noTone(BUZZER);
    }
